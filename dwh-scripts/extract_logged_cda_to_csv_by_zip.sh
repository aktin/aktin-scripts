#!/bin/bash
#--------------------------------------
# Script Name:  extract_logged_cda_to_csv_by_zip.sh
# Version:      1.1
...
#               specific medical data fields, filters based on allowed postal codes, and saves the results into a timestamped
#               CSV file while logging errors.
#--------------------------------------
...
# Zipcodes to filter the CDAs by
ALLOWED_POSTAL_CODES=("80636" "52224")
...
parse_cda_file() {
    ...
    # Skip the file if the postal code is not allowed
    if ! is_postal_code_allowed "$postal_code"; then
        echo "Skipping file $cda_file: Postal code $postal_code not allowed." >> "$LOG_FILE"
        return 1
    fi
